...
class HHruParser:
    ...
    def item_vacancy(self, content, link):
        ...
        try:
            ...
            name = None
        vacancy_href = link
        salary_raw = soup.find(
            'span', {'data-qa': 'vacancy-salary-compensation-type-gross'})
        if salary_raw:
            sal_raw = salary_raw.text.replace('\xa0', '')
    ...
    def get_vacansy(self, content):
        ...
        vacancies = []
        for i in block:
            link_vacansy = i.findChild(
                'a', {'class': 'serp-item__title'}).get('href')
            response = requests.get(link_vacansy, headers=self.headers)
            vacancies.append(self.item_vacancy(response, link_vacansy))
        return vacancies
